...
- _Text embeddings_ : [**représentation dense**]{.orange} qui capture les grandes structures d'un corpus.
- Projection du corpus dans le meilleur espace vectoriel possible pour rapprocher les termes sémantiquement proches. 
- Exemple de [**word2vec**]{.orange} (2013), apprentissage d'embeddings à l'aide d'une tâche de prédiction des mots à partir de leur contexte
...
- Représentations basées sur :
  - [**mots**]{.orange} 
  - [**n-grams de caractères**]{.orange}  (ex. « tar », « art », « rte »)
  - [**n-grams de mots**]{.orange} 
- Avantages :
  - robuste aux [**variations morphologiques**]{.blue} 
...
![](img/fasttext_shema.png){.lightbox}
...
- Explosion des modèles de langage [**génératifs**]{.orange}
...
- Exploitation des données Sirene avec {{< fa brands python >}} et `TorchTextClassifiers` (méthode Fasttext)
